"""
BAMRC annotation pipeline using DeepSeek Chat.
...
Output: s12_deepseek_bamrc.csv, s12_deepseek_bamrc.txt, s12_deepseek_bamrc_log.csv
"""
...
OUTPUT_CSV = 's12_deepseek_bamrc.csv'
OUTPUT_TXT = 's12_deepseek_bamrc.txt'
...
def get_deepseek_response(user_prompt, max_retries=3, retry_delay=3):
...
def process_abstracts_with_deepseek(df):
    ...
    for index, row in df.iterrows():
        ...
        annotated = get_deepseek_response(f'Abstract: {abstract}')
...
def main():
    print('BAMRC Annotation Pipeline - DeepSeek Chat')

    df = load_abstracts_as_dataframe(INPUT_FILE)
    if df is None:
        return

    df_processed = process_abstracts_with_deepseek(df)
