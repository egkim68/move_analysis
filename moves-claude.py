...
Output: s12_claude_bamrc.csv, s12_claude_bamrc.txt, s12_claude_bamrc_log.csv
"""
...
OUTPUT_CSV = 's12_claude_bamrc.csv'
OUTPUT_TXT = 's12_claude_bamrc.txt'
...
def get_claude_response(user_prompt, max_retries=3, retry_delay=3):
...
def process_abstracts_with_claude(df):
    ...
    for index, row in df.iterrows():
        ...
        annotated = get_claude_response(f'Abstract: {abstract}')
...
def main():
    print('BAMRC Annotation Pipeline - Claude 3.5 Haiku')

    df = load_abstracts_as_dataframe(INPUT_FILE)
    if df is None:
        return

    df_processed = process_abstracts_with_claude(df)
