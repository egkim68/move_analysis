...
Output: s12_openai_bamrc.csv, s12_openai_bamrc.txt, s12_openai_bamrc_log.csv
"""
...
OUTPUT_CSV = 's12_openai_bamrc.csv'
OUTPUT_TXT = 's12_openai_bamrc.txt'
...
def get_openai_response(user_prompt, max_retries=3, retry_delay=3):
...
def process_abstracts_with_openai(df):
    ...
    for index, row in df.iterrows():
        ...
        annotated = get_openai_response(f'Abstract: {abstract}')
...
def main():
    print('BAMRC Annotation Pipeline - OpenAI GPT-4o Mini')

    df = load_abstracts_as_dataframe(INPUT_FILE)
    if df is None:
        return

    df_processed = process_abstracts_with_openai(df)
